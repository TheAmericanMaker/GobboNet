Read .codecarto/GUIDE.md and continue the CodeCartographer workflow for the phase `porting`.
...
- .codecarto/findings/porting/reverse-engineering-bundle.md if it already exists (continue instead of duplicating work)
...
Items routed to `porting` for closure (carry_forward from earlier phases):
- arch-CF6 (defer-to-phase) Portability hazard inventory: PowerShell HttpListener + runspaces, cmd batch orchestration (taskkill/tasklist/start/netsh), browser storage + Page Lifecycle APIs, mDNS/.local origin stability, Windows-only System32 tooling, AV-avoidance download patterns.
...
- This phase declares secondary outputs. Each should end the phase either written or explicitly accounted for (in Coverage and limits, or a routed handoff entry) — never dropped silently:
  ...
  - .codecarto/findings/build-and-deploy/build-and-deploy.md (exists)
...
- For long phases, checkpoint resumable progress at .codecarto/scratch/checkpoints/porting.md; Pi writes this automatically after phase compaction.
...
- On completion, write a phase handoff to .codecarto/scratch/handoffs/porting.yaml (see GUIDE.md). Do NOT directly edit workflow/status.yaml, append THREAD_LOG.md, or create a second closeout.
...
Primary output target: .codecarto/findings/porting/reverse-engineering-bundle.md
