Read .codecarto/GUIDE.md and continue the CodeCartographer workflow for the phase `contracts`.
...
- .codecarto/findings/contracts/behavioral-contracts.md if it already exists (continue instead of duplicating work)
- .codecarto/findings/contracts/SKILL.md
- .codecarto/templates/behavioral-contracts.md
...
Items routed to `contracts` for closure (carry_forward from earlier phases):
- arch-CF2 (defer-to-phase) User-visible behavior contracts for the README feature list (streaming, reroll, branching, lore compression, macros, scheduler, hot-swap, data export/import, extensions) — triggers, defaults, outputs, side effects, error behavior.
...
- For long phases, checkpoint resumable progress at .codecarto/scratch/checkpoints/contracts.md; Pi writes this automatically after phase compaction.
...
- On completion, write a phase handoff to .codecarto/scratch/handoffs/contracts.yaml (see GUIDE.md). Do NOT directly edit workflow/status.yaml, append THREAD_LOG.md, or create a second closeout.
...
Primary output target: .codecarto/findings/contracts/behavioral-contracts.md
